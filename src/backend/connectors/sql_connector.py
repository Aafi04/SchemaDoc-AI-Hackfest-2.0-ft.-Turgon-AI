...
class SQLConnector:
    ...
    def get_live_schema(self) -> Dict[str, TableSchema]:
        ...
        print(f"Connected to DB. Found tables: {table_names}")
        ...
        for t_name in table_names:
            try:
                ...
                # get structural data like primary n foreign keys stuff
                ...
                schema_out[t_name] = {
                    ...
                    "description": None, # just a placeholder for AI for now
        ...
        try:
            with self.engine.connect() as conn:
                ...
                for col_name, meta in cols_meta.items():
                    ...
                    if any(t in type_str for t in ["INT", "FLOAT", "DECIMAL", "NUMERIC", "REAL"]):
                        try:
                            stats_q = select(func.min(col_obj), func.max(col_obj), func.avg(col_obj))
